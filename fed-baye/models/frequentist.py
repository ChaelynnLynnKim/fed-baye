...
def make_frequentist_recommender(
    ...
    """
    Builds recommender system with dense neural network sub-models

    Builds recommender system with dense neural network-based
    sub-models using TensorFlow Probability and TensorFlow's
    functional API. Three separate blocks in the model -
    one for user embeddings, one for movie embeddings, and one
    for prediction of user-movie ratings.
    ...
    # batch normalization and blocks of dense layers, each with its own
    # batch normalization
    ...
    # sub-models, adds additional dense layers with batch normalization,
